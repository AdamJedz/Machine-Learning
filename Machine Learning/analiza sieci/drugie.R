...
layout1 <- layout.fruchterman.reingold(Prosty_Graph)
#ustawianie kolorów
...
#kolejne dane
data <- read.csv('X:/siec/networkdata.csv')
...
#zamiast fruchterman.reingold wpisz:
#graphopt, kamada.kawai
set.seed(200)
plot(net,
     vertex.color=rainbow(52),
     vertex.size=V(net)$degree*0.4,
...
plot(net,
     ...
     edge.arrow.size=0.1,
     layout=layout.kamada.kawai)
#drzewko - tutaj troche za duzo danych
set.seed(200)
plot(net,
     vertex.color="grey",
     edge.arrow.size=0.1,
     layout=layout.reingold.tilford(net,root="CA")) #root = CC lub CA

#betweenness
betweenness(net)
plot(net,
     vertex.color="green",
     vertex.size= sqrt(betweenness(net)), #sqrt - pierwiastek zeby zmniejszyc kropeczki
     edge.arrow.size=0.1,
     layout=layout.kamada.kawai)
#opcja niter
plot(net,
...
#usuniecie petli oraz wielokrotnych polaczen
net2 <- simplify(net, remove.multiple = TRUE, remove.loops = TRUE)
plot(net2,
     vertex.color = "grey",
     vertex.size = sqrt(betweenness(net2)),
     edge.arrow.size=0.1,
